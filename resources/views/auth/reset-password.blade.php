@extends('layouts.auth', ['title' => 'Update Password - Member'])
...
@section('content')
    ...
    <main class="bg-white max-w-lg mx-auto p-8 md:p-12 my-10 rounded-lg shadow-2xl">
        <section>
            <h3 class="font-bold text-2xl text-center">Update Password</h3>
        </section>
        @if (session('status'))
        ...
        <section class="mt-10">
            <form class="flex flex-col" method="POST" action="{{ route('password.update') }}">
                ...
                <div class="mb-6 pt-3 rounded bg-gray-200">
                    ...
                    <input type="email" name="email" id="email" value="{{ $request->email ?? old('email') }}" class="bg-gray-200 rounded w-full text-gray-700 focus:outline-none border-b-4 border-gray-300 focus:border-indigo-600 transition duration-500 px-3 pb-3" required>
                    @error('email')
                ...
                <div class="mb-6 pt-3 rounded bg-gray-200">
                    <label class="block text-gray-700 text-sm font-bold mb-2 ml-3" for="password_confirmation">Email</label>
                    <input type="password" name="password_confirmation" id="password_confirmation" class="bg-gray-200 rounded w-full text-gray-700 focus:outline-none border-b-4 border-gray-300 focus:border-indigo-600 transition duration-500 px-3 pb-3" required>
